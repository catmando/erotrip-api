class GroupsJoinModal < Hyperloop::Component
    ...
    span do
      div(class: 'modal-body text-center') do
        ...
        div(class: 'mt-4 mb-4 flex justify-content-center align-items-center') do
          button(class: "mr-4 btn btn-ouline-gray-to-primary btn-round-125 #{'active' if state.public == true}") do
            'Publicznie'
          end.on :click do
            mutate.public true
          end
          button(class: "btn btn-ouline-gray-to-primary btn-round-125 #{'active' if state.public == false}") do
            'Anonimowo'
          end.on :click do
            mutate.public false
          end
  ...
  def join_group
    ...
    SaveUserGroup.run(user_id: CurrentUserStore.current_user.id, group_id: params.group.try(:id), public: state.public)
    .then do |saved|
      mutate.blocking false
      `toast.success('Dołączyłeś/aś do grupy!')`
